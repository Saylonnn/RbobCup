...
class ThreadingServer():
    ...
    def run_cam(self):
        # ---------------------------------------------------------------------------------------------------------------------------------------
        green_temp = cv2.imread('green_temp.jpg', cv2.IMREAD_COLOR).astype(np.uint8)
        black_temp = cv2.imread('black_temp.jpg', cv2.IMREAD_COLOR).astype(np.uint8)
        blue_temp = cv2.imread('blue_temp.jpg', cv2.IMREAD_COLOR).astype(np.uint8)
        yellow_temp = cv2.imread('yellow_tmp.jpg', cv2.IMREAD_COLOR).astype(np.uint8)
        ball_temp = cv2.imread('ball_temp.jpg', cv2.IMREAD_COLOR).astype(np.uint8)
        ...
        while (nRet == ueye.IS_SUCCESS):
            ...
            resBlue = cv2.matchTemplate(frame, blue_temp, 5)
            blue_min_val, blue_max_val, blue_min_loc, blue_max_loc = cv2.minMaxLoc(resBlue)
            blue_top_left = blue_max_loc
            blue_bottom_right = (blue_top_left[0] + 17, blue_top_left[1] + 17)
            cv2.rectangle(frame, blue_top_left, blue_bottom_right, (100,100,100), 2)

            resGreen = cv2.matchTemplate(frame, green_temp, 5)
            ...
            cv2.rectangle(frame, green_top_left, green_bottom_right, (0,255,0), 2)


            resBall = cv2.matchTemplate(frame, ball_temp, 5)
            ball_min_val, ball_max_val, ball_min_loc, ball_max_loc = cv2.minMaxLoc(resBall)
            ball_top_left = ball_max_loc
            ball_bottom_right = (ball_top_left[0] + 17, ball_top_left[1] + 17)
            cv2.rectangle(frame, ball_top_left, ball_bottom_right, (0,0,255), 2)

            resBlack = cv2.matchTemplate(frame, black_temp, 5)
            black_min_val, black_max_val, black_min_loc, black_max_loc = cv2.minMaxLoc(resBlack)
            black_top_left = black_max_loc
            black_bottom_right = (black_top_left[0] + 17, black_top_left[1] + 17)
            cv2.rectangle(frame, black_top_left, black_bottom_right, (0,0,0), 2)

            resYellow = cv2.matchTemplate(frame, yellow_temp, 5)
            yellow_min_val, yellow_max_val, yellow_min_loc, yellow_max_loc = cv2.minMaxLoc(resYellow)
            yellow_top_left = yellow_max_loc
            yellow_bottom_right = (yellow_top_left[0] + 17, yellow_top_left[1] + 17)
